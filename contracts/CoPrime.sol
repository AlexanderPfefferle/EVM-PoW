...
contract CoPrime {
    ...
    constructor(uint _a, uint _b, bytes32 _target) payable {
        require (msg.value >= 0.1 ether, "insufficient rewards");
        require(_a > _b && _a % _b != 0, "assuming a>b and a%b != 0");
        owner = msg.sender;
        a = _a;
        b = _b;
        target = _target;
    ...
    function offChain(uint rand) public view returns (bool isCounterexample, bytes32 _hashValue) {
        ...
        uint potential_divisor = rand % (b / 2 - 2) + 2;
        isCounterexample = (a % potential_divisor == 0) && (b % potential_divisor == 0);
