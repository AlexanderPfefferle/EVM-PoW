...
def set_leading_zeros(x): return "{0:0{1}x}".format(x, 4).upper()
    ...
    for x in range(1, len(contract)):
        ...
        if (opcodes[op]["name"] in {"JUMP", "JUMPI"}):
            if opcodes[p_op]["name"][:4] == "PUSH":
                contract[x -
                         1][2] = old_address_to_label[int(''.join(p_params), 16)]
            else:
...
def fix_constructor(constructor, new_code_size):
    ...
    for x in range(len(constructor)):
        ...
        if constructor[x][2] == "CS":
            ...
            new_constructor_bytecode += set_leading_zeros(
                new_constructor_length)
        elif constructor[x][2] == "CL+CS":
            new_constructor_bytecode += set_leading_zeros(
                new_constructor_length+new_code_size)
        else:
...
if __name__ == '__main__':
    arg_parser = argparse.ArgumentParser(
        description="Modify EVM Bytecode such that it computes a hash unique to the executed code.")
    ...
    contract_bytes = [bytecode[i]+bytecode[i+1]
    ...
    # assign labels
    jumppc_to_pushpc, blocks_list, func_to_blocks = run_and_parse_vandal(
        reassemble(contract))
    old_address_to_label = assign_labels(contract, jumppc_to_pushpc)
    # modify code
    block_ranges = find_offchain_blocks(
        func_to_blocks, blocks_list, args.functionhash.lower())
    internal_function_call_returns = find_internal_call_returns(
        contract, old_address_to_label, block_ranges)
    contract = inject_code(contract, block_ranges,
                           internal_function_call_returns)
    make_pushes_big(contract)
    # remove labels
    new_bytecode = reassemble(contract)
    cc = fix_constructor(constructor, int(
        hex(len(new_bytecode)//2)[2:].upper(), 16))
