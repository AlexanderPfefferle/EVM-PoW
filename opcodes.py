...
#PUSHs
for i in range(1,33):
    opcodes[hex(int("5F",16)+i)[2:].upper()]={"name":'PUSH'+str(i),"num_stack_in":0, "num_stack_out":1,"extra_in":i}
#SWAPs
for i in range(1,17):
    opcodes[hex(int("7F",16)+i)[2:].upper()]={"name":'DUP'+str(i),"num_stack_in":-1, "num_stack_out":1,"extra_in":0}
#DUPs
for i in range(1,17):
    opcodes[hex(int("8F",16)+i)[2:].upper()]={"name":'SWAP'+str(i),"num_stack_in":-1, "num_stack_out":-1,"extra_in":0}

#invalids
for op_index in range(256):
    op_hex=hex(op_index)[2:].upper().zfill(2)
    if op_hex not in opcodes.keys():
        opcodes[op_hex]={"name":'INVALID',"num_stack_in":-1, "num_stack_out":-1,"extra_in":0}

opcodes['-1']={"name":'undefined',"num_stack_in":-1, "num_stack_out":-1,"extra_in":0}
